...
- **创作声明**：补丁让 `set_self_declaration` 可配，smu 默认选「内容由AI生成」
  （`--no-ai-statement` 关）。取值须与抖音弹窗文字完全一致。
- **封面比例**：抖音竖封面 **3:4**、横封面 **4:3**（不是 16:9）。smu 按真实宽高比检测选图，
  不靠文件名，比例对不上的不传。
- **封面槽顺序**：sau 的 `set_thumbnail` 标签页顺序与当前抖音 UI 相反，smu 用
  `_DOUYIN_COVER_SWAP=True` 反向传参修正；若 sau 修了顺序，把该常量改 False。
