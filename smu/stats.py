...
各平台都用 cookie 直接拉创作中心接口（比 DOM 抓取稳）：
  - douyin：GET creator.douyin.com/web/api/media/aweme/post/ → 每条 statistics（实测可用）
  - bilibili：GET member.bilibili.com/x/web/archives → 每稿 stat（实测可用）
  - xiaohongshu / shipinhao：创作中心接口多需 x-s 签名，待接入（见 _TODO_URLS）
...
# 待接入平台的创作中心数据页（参考 MatrixFlow，后续做 DOM 抓取或找签名接口）
_TODO_URLS = {
    "xiaohongshu": "https://creator.xiaohongshu.com/datacenter/content",
